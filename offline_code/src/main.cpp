  ...
  /*	if(argc!=8)
	{
		...
		printf("\nex.  ./src/yolov3_offline_simple_demo model/yolov3.cambricon subnet0 0 0 data/000000581357.jpg 2 data/label_map_coco.txt\n\n");
		return -1;
	}*/
	...
	const char 				*makeup_image_path=argv[6];		//测试图片的路径
	int                     is_rgb=atoi(argv[7]);       //数据预处理选项
	int                     is_rgb_make=atoi(argv[8]);       //数据预处理选项
//    string                  label_filename(argv[8]);    //label文件
...
//    void 					**inputNHWCCpuPtrS;				//输入数据的CPU指针
    ...
    //inputNHWCCpuPtrS = (void **)malloc(sizeof(void *) * inputNum);
	...
	{	
		...
		printf("\n");
		//printf("input size:%d\n", inputSizeS[i]);
                //inputNHWCCpuPtrS[i]=(void *)malloc(count*sizeof(float));
                input_count.push_back(count);
	...
	{		
	  ...
	  input_image.convertTo(input_image, CV_32F);		
          ...
          cv::Mat sample_temp_rgb(input_image.cols, input_image.rows, CV_32FC3);
          cv::cvtColor(input_image, sample_temp_rgb, CV_BGR2RGB);
          float img_w = input_image.cols;
          float img_h = input_image.rows;
          float img_scale = img_w < img_h ? (input_height / img_h) : (input_width / img_w);
          int new_w = std::floor(img_w * img_scale);
          ...
          cv::Mat sample_temp;
          cv::resize(sample_temp_rgb, sample_temp, cv::Size(new_w, new_h), CV_INTER_LINEAR);
          ...
          sample_temp.copyTo(net_input_data_rgba(
                                             ...
                                             cv::Range((static_cast<float>(input_width) - new_w) / 2,
                                                       (static_cast<float>(input_width) - new_w) / 2 + new_w)));
          ptr+=(input_height*input_width*12);
        }
    }
	int input_idx_markup=1;

        cv::Mat makeup_imgs = cv::imread(makeup_image_path);

	unsigned char *ptr_makeup=(unsigned char *)inputCpuPtrS[input_idx_markup];
	for(int i=0;i<batch_size;i++)
	{		
	  if(is_rgb_make==2) {
          cv::Mat sample_temp;

          float img_w = makeup_imgs.cols;
          float img_h = makeup_imgs.rows;
          cv::Mat sample_temp_bgr(makeup_imgs.cols, makeup_imgs.rows, CV_8UC4);
          float img_scale = img_w < img_h ? (input_height / img_h) : (input_width / img_w);
          int new_w = std::floor(img_w * img_scale);
          int new_h = std::floor(img_h * img_scale);
		  cv::cvtColor(makeup_imgs, sample_temp_bgr, CV_BGR2RGBA);
          cv::resize(sample_temp_bgr, sample_temp, cv::Size(new_w, new_h), CV_INTER_LINEAR);
          
          cv::Mat net_input_data_rgba(input_height,input_width,CV_8UC4,ptr_makeup);
          sample_temp.copyTo(net_input_data_rgba(
                                             cv::Range((static_cast<float>(input_height) - new_h) / 2,
                                                       (static_cast<float>(input_height) - new_h) / 2 + new_h),
                                             cv::Range((static_cast<float>(input_width) - new_w) / 2,
                                                       (static_cast<float>(input_width) - new_w) / 2 + new_w)));
          ptr_makeup +=(input_height*input_width*4);
        }  else if(is_rgb_make==3) {

	  makeup_imgs.convertTo(makeup_imgs, CV_32F);		
          makeup_imgs = (makeup_imgs - 127.5) / 127.5;
          cv::Mat sample_temp_rgb(makeup_imgs.cols, makeup_imgs.rows, CV_32FC3);
          cv::cvtColor(makeup_imgs, sample_temp_rgb, CV_BGR2RGB);
          ...
          cv::Mat net_input_data_rgba(input_height,input_width,CV_32FC3,ptr_makeup);
          sample_temp.copyTo(net_input_data_rgba(
                                             cv::Range((static_cast<float>(input_height) - new_h) / 2,
                                                       (static_cast<float>(input_height) - new_h) / 2 + new_h),
                                             cv::Range((static_cast<float>(input_width) - new_w) / 2,
                                                       (static_cast<float>(input_width) - new_w) / 2 + new_w)));
          ptr_makeup+=(input_height*input_width*12);
        }
    }

/*        ifstream infile("relu5.nptxt",ios::in);
        float *ptr_makeup=(float *)inputCpuPtrS[input_idx_markup];
        for(int i=0;i<input_count[input_idx_markup];i++)
        {
            infile>>ptr_makeup[i];
            //ptr_makeup[i] = 0.0;
//            printf("num:%d, content:%f\n", i, ptr_makeup[i]);
        }*/
	auto t0=GetTickCount();
	//拷贝输入数据到MLU内存
        /*if(false){int dim_order[4] = {0, 2 , 3, 1};
	CNRT_CHECK(cnrtGetInputDataShape(&dimValues,&dimNum,input_idx_markup,function));
	
		//NHWC->NCHW half->float32
		CNRT_CHECK(cnrtTransOrderAndCast(reinterpret_cast<void*>(inputCpuPtrS[input_idx_markup]), inputTypeS[input_idx_markup],
										 reinterpret_cast<void*>(inputNHWCCpuPtrS[input_idx_markup]), CNRT_FLOAT32,
	...
	CNRT_CHECK(cnrtMemcpy(inputMluPtrS[input_idx_markup], inputCpuPtrS[input_idx_markup],inputSizeS[input_idx_markup],CNRT_MEM_TRANS_DIR_HOST2DEV));
	...
	//打印输出结果
	float *output_ptr=(float*)outputCpuPtrS[output_idx];
        //printf("%d,%d\n",output_idx, output_count[output_idx]); 
        for (int i =0; i < output_count[output_idx]; i++) {
            output_ptr[i] = (output_ptr[i] + 1.0) * 127.5;
        }
        
        /*printf("\n"); 
        for (int i =0; i < output_count[output_idx]; i++) {
            printf("%d:%f,",i, output_ptr[i]);
        }
        printf("\n"); */

       unsigned char cbuf[3 * 256 * 256];
        for (int i =0; i < output_count[output_idx]; i++) {
            cbuf[i] = (unsigned char)output_ptr[i];
            //printf("%d:%u,",i, cbuf[i]);
        }
       cv::Mat img(256 , 256, CV_8UC3, (unsigned char*)cbuf);
      cv::Mat color_bgr;
      cvtColor(img, color_bgr, COLOR_RGB2BGR);

      cv::imwrite( "haha.jpg" , color_bgr);     
